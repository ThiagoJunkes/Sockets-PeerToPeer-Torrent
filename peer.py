...
class Peer: 
    ...
    def download_files(client):
        ...
        while True:
            ...
            rarest_file = Peer.get_rarest_file(files_to_download)
            if rarest_file is None:
                print("No rarest file found.")
                break

            print(f"Downloading {rarest_file}")
            peer_ip = Peer.select_peer_with_file(rarest_file)
            if(peer_ip == None):
    ...
    def reciving_connection():
        ...
        while True:
            ...
            thread_peer = threading.Thread(target=Peer.handle_peer, args=(conn, addr))
            thread_peer.start()
    ...
    def main():
        ...
        thread_send_files = threading.Thread(target=Peer.send_files, args=(client,))
        thread_send_files.start()

        thread_peer_connection = threading.Thread(target=Peer.reciving_connection)
